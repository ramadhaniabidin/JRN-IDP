...
namespace Daikin.BusinessLogics.Common
{
    ...
    public class SharePointManager
    {
        ...
        public string CopyListAttachmentToDocLib(string WebUrl, string listName,
            string DocLibName, string FolderNameWithDelimiterPipeline, int ItemId)
        {
            try
            {
                string ParentSiteUrl = WebUrl;
                ...
                SPSecurity.RunWithElevatedPrivileges(delegate ()
                {
                    ...
                    using (SPSite oSPsite = new SPSite(WebUrl))
                    {
                        using (SPWeb oSPWeb = oSPsite.OpenWeb())
                        {
                            ...
                            //string fileName = Path.GetFileName(AttachmentListUrl);

                            //SPFile file = oSPWeb.GetFile(AttachmentListUrl);
                            //byte[] binFile = file.OpenBinary();
                            #endregion

                            string destUrl = string.Empty;

                            string InitWebUrl = WebUrl;

                            //Activate when single upload
                            //DocUrl = UploadDocLib(binFile, ParentSiteUrl, DocLibName, destUrl);


                            //Loop through each list item (BULK)
                            //Get all attachments
                            SPAttachmentCollection attachmentsColl = item.Attachments;

                            foreach (string attachment in attachmentsColl)
                            {
                                #region Create folder with subfolders
                                string fileName = attachment;
                                WebUrl = InitWebUrl;
                                if (FolderNameWithDelimiterPipeline.Contains("|"))
                                {
                                    string[] folders = FolderNameWithDelimiterPipeline.Split('|');
                                    if (!WebUrl.EndsWith("/"))
                                    {
                                        WebUrl += "/" + DocLibName;
                                    }
                                    ...
                                    foreach (string f in folders)
                                    {
                                        ...
                                        {
                                            WebUrl += "/" + f;
                                            ensureParentFolder(WebUrl + "/" + fileName, ParentSiteUrl);
                                        }
                                    }
                                    destUrl = WebUrl + "/" + fileName;
                                }
                                else
                                {

                                    destUrl = WebUrl + DocLibName + "/" + FolderNameWithDelimiterPipeline + "/" + fileName;
                                    ensureParentFolder(destUrl, ParentSiteUrl);
                                }
                                ...
                                byte[] binFile = file.OpenBinary();
                                UploadDocLib(binFile, ParentSiteUrl, DocLibName, destUrl);
