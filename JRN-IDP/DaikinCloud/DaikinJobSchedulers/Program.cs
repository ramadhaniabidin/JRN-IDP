...
namespace Daikin.JobSchedulers
{
    class Program
    {
        ...
        static bool isBatchFeedbacks(string F_Code)
        {
            return batchFeedbackActions.Contains(F_Code);
        }
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                #region Batch Feedback
                else if (isBatchFeedbacks(F_Code))
                {
                    BatchFeedbackActions(F_Code);
                }
                #endregion
        ...
        static void BatchFeedbackActions(string F_Code)
        {
            switch (F_Code)
            {
                case "33":
